 ...
 */
    static v8::Local<v8::Value> _pack(const v8::FunctionCallbackInfo<v8::Value>& argv)
    {
        ...
        // char localOutput[256]; wrong way,node will free the char * data ,so should use malloc or new ;
        char * localOutput = new char[256];
...
#define GETARG(n)	(argsFromArray ? args->Get(n) : argv[n])
        ...
        v8::Local<v8::Object> return_buffer = node::Buffer::New( isolate,output, outputpos).ToLocalChecked();
        if( outputsize > 255 ) {
            free(output);
        }
        if( IsDebug()>0 ) {
